...
export default function EditFind() {
  ...
  const [formData, setFormData] = useState({
    id: "", name: "", area: "", notes: "", latitude: null as number | null, longitude: null as number | null, plusCode: "", photoPath: "", gallery: [] as string[]
  });
  ...
  useEffect(() => {
    fetch("/api/mushrooms")
      ...
      .then(data => {
        ...
        if (item) {
          const gallery = item.gallery ? (typeof item.gallery === "string" ? JSON.parse(item.gallery) : item.gallery) : [];
          if (item.photoPath && gallery.length === 0) {
            gallery.push(item.photoPath);
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    const payload = { ...formData, gallery: JSON.stringify(formData.gallery) };
    await fetch("/api/mushrooms", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    router.push("/");
  };

  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to completely delete this find? This cannot be undone.")) {
      await fetch("/api/mushrooms", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
      router.push("/");
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 text-gray-900 pb-24">
      <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-emerald-800">🍄 Mushroom Details</h1>
          <button type="button" onClick={handleDelete} className="text-red-600 hover:text-red-800 p-2 rounded-lg hover:bg-red-50 text-sm font-bold flex items-center gap-1 transition-colors">
            🗑️ Delete
          </button>
        </div>
        ...
        <form suppressHydrationWarning onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button type="submit" disabled={submitting} className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition-colors">{submitting ? "Save Changes" : "Save Changes"}</button>
          </div>
        </form>
      </div>
    </div>
